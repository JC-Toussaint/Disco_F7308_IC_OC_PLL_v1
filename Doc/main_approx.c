...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	printf("start\n");
	// TIM9_freq =108 MHz
	// T = (ARR+1)*(PSC+1)/108e6=  avec ARR=49999 et PSC=215
	// Pulse=24999 => freq = 20 Hz
	// T = (ARR+1)*(PSC+1)/108e6=  avec ARR=4999 et PSC=215
	// Pulse=2499 => freq = 200 Hz
	// T = (ARR+1)*(PSC+1)/108e6=   avec ARR=499 et PSC=215
	// Pulse=249 => freq = 2000 Hz
	// T = (ARR+1)*(PSC+1)/108e6=   avec ARR=499 et PSC=215
	// Pulse=249 => freq = 2000 Hz
	// T = (ARR+1)*(PSC+1)/108e6=   avec ARR=26 et PSC=7
	// Pulse=13 => freq = 20000 Hz
	// T = (ARR+1)*(PSC+1)/108e6=6.37037e-06  avec ARR=15 et  PSC=42
	// Pulse=3  => freq = 156976 Hz
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
		if (CH1_captureDone != 0) {
			...
			float fperiod = (float) CH1_diffCapture*ratio + fperiodErr;
			period = (uint32_t) fperiod;
			fperiodErr = fperiod - period;
			TIM2->CCR2 = TIM2_counter + period;
			CH1_captureDone=0;
			/*
			TIM9->ARR++;
			TIM9->CCR1 = TIM9->ARR >> 1;
		...
		// assumption of an even divisor
		  if (DIV_counter >= (DIVIDER>>1)){
...
// Set selected pins that were at low level, and reset ones that were high
 ...
 void HAL_TIM_OC_DelayElapsedCallback(TIM_HandleTypeDef *htim) {
  if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2){
	  TIM2->CCR2 += period;
  	  if (!CH1_captureDone) HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_5);
  }
}
